...
def tree_matmul_tree_matsum(bm1, bm2, mul_splits: int, sum_splits: int = None,
    ...
    if not read_if_exists:
        ...
        split_points = list(range(0, bm1._n_block_cols, inner_brange_size)) + [bm1._n_block_cols]
        len(list(range(0, 7078, int(math.ceil(7078/100)))) + [7078])
        print(split_points)
        inner_ranges = list(zip(split_points[:-1], split_points[1:]))
        print(f'len(inner_ranges): {len(inner_ranges)}')
    ...
    for i in range(sum_splits): #range(sum_splits):
        # partial_sum_path = f"{path_prefix}-partial-{i}"
        print(read_intermediates[i*sum_block_size:(i+1)*sum_block_size])
        tracked_partial_sums.append(BlockMatrix.read(partial_sum_path))
...
def main(args):
    ...
    #     mt = mt.key_cols_by(*PHENO_KEY_FIELDS)
    #     ht = hl.read_table('gs://ukb-diverse-pops/ld_prune/joined_ukbb_lancet_age_high_prev_pan_ancestry.ht')
    ...
    #     clump_sumstats_col_ht_path = get_clump_sumstats_col_ht_path(high_quality=args.high_quality,
    #                                                                 max_pops=args.max_pops).replace('/prs/',f'/prs/{test_prefix}')
    #     print(f'clump_sumstats_col_ht_path: {clump_sumstats_col_ht_path}')
    #     mt.add_col_index('idx').key_cols_by('idx').cols().write(
    #         clump_sumstats_col_ht_path, 
    #         args.overwrite)
    #     clump_sumstats_bm_path = get_clump_sumstats_bm_path(high_quality=args.high_quality,
    #                                                         max_pops=args.max_pops).replace('/prs/',f'/prs/{test_prefix}')
    #     print(f'clump_sumstats_bm_path: {clump_sumstats_bm_path}')
    ...
    if args.compute_prs:
        ...
        #                                                     max_pops=args.max_pops).replace('/prs/',f'/prs/{test_prefix}')
        # clump_sumstats_bm_path = get_clump_sumstats_bm_path(high_quality=args.high_quality,
        #                                                     max_pops=args.max_pops).replace('/prs/',f'/prs/high_prev_test.')
        # sumstats_bm = BlockMatrix.read(clump_sumstats_bm_path)
        # genotype_bm = BlockMatrix.read(genotype_bm_path)
        # prs_bm = genotype_bm.T.tree_matmul(sumstats_bm, splits=100, 
        #                                    path_prefix = f'{temp_bucket}/prs/{test_prefix}tree_matmul{"_max_pops" if args.max_pops else ""}')
        ...
        # print(f'mul_splits: {mul_splits}\nsum_splits: {sum_splits}')
        # prs_bm = tree_matmul_tree_matsum(genotype_bm.T, sumstats_bm, mul_splits=mul_splits, 
        #                                   sum_splits=sum_splits, path_prefix = f'{temp_bucket}/prs/{test_prefix}tree_matmul{"_max_pops" if args.max_pops else ""}0',
        #                                   read_if_exists = True)
        path_prefix = f'{temp_bucket}/prs/{test_prefix}tree_matmul{"_max_pops" if args.max_pops else ""}'
        # prs_bm = genotype_bm.T.tree_matmul(sumstats_bm, splits=mul_splits, path_prefix = path_prefix)
        read_intermediates = [BlockMatrix.read(f"{path_prefix}_{i}") for i in range(0, mul_splits)]
        prs_bm = sum([sum(read_intermediates[:50]), sum(read_intermediates[50:])])
        prs_bm_path = get_prs_bm_path(high_quality=args.high_quality,
                                      max_pops=args.max_pops).replace('/prs/',f'/prs/{test_prefix}')
    ...
    if args.create_prs_mt:
        prs_bm_path = get_prs_bm_path(high_quality=args.high_quality,
                                      max_pops=args.max_pops).replace('/prs/',f'/prs/{test_prefix}')
        prs_bm = BlockMatrix.read(prs_bm_path)
        clump_sumstats_col_ht_path = get_clump_sumstats_col_ht_path(high_quality=args.high_quality,
                                                                    max_pops=args.max_pops).replace('/prs/',f'/prs/{test_prefix}')
        pheno_ht = hl.read_table(clump_sumstats_col_ht_path).key_by('idx')
        ...
        prs_mt_path = get_prs_mt_path(high_quality=args.high_quality, 
                                      max_pops=args.max_pops).replace('/prs/',f'/prs/{test_prefix}')
        mt.write(prs_mt_path, args.overwrite)

    if args.assess_prs:
        prs_mt_path = get_prs_mt_path(high_quality=args.high_quality, 
                                      max_pops=args.max_pops).replace('/prs/',f'/prs/{test_prefix}')
        prs_mt = hl.read_matrix_table(prs_mt_path)
        ...
        for pop in POPS:
            ...
            cols.select('description', 
                        ...
                        prs_corr_n=cols.prs_corr.n).export(f'gs://ukbb-diverse-temp-30day/prs/{test_prefix}assess_prs{"_max_pops" if args.max_pops else ""}.{pop}.tsv.gz')
